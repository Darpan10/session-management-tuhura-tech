    ...
    <div ref={ref} className={`bg-white rounded-lg shadow-md overflow-hidden ${isHighlighted ? 'ring-4 ring-blue-500' : ''}`}>
      <div className={`px-6 py-4 ${colorClass} text-white`}>
        <div className="flex items-center justify-between">
          ...
          {students.length > 0 && (
            <button
              onClick={() => handleSelectAll(students, selectedSet, setSetter)}
                ...
                <tr key={student.id} className="hover:bg-gray-50">
                  <td className="px-6 py-4 whitespace-nowrap">
                    <input
                      ...
                      onChange={() => handleToggleSelection(student.id, selectedSet, setSetter)}
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-gray-600">
                    {session.dayOfWeek} {session.startTime} - {session.endTime} | {session.termNames?.join(', ')}
                  </p>
                </div>
