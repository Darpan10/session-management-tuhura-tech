...
import type { SessionWithCount, AllSessionsStudentCountResponse } from '../../types/session';
...
const GlobalStudentManagement: React.FC = () => {
  ...
  const loadData = async () => {
    try {
      ...
      const response: AllSessionsStudentCountResponse = await sessionAPI.getAllSessionsWithStudentCounts(status);
      
      console.log('API Response:', response);
      console.log('Sessions:', response.sessions);
      console.log('First session:', response.sessions[0]);
        ...
        <div className="p-8">
          ...
          <div className="mb-6 bg-white rounded-lg shadow-md p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-gray-600 mb-1">Total {getStatusConfig().title}</p>
                <p className="text-3xl font-bold text-gray-900">{totalStudents}</p>
              </div>
              <div className={`${getStatusConfig().color} text-white p-4 rounded-full`}>
          ...
          {isLoading ? (
            ...
            <div className="bg-white rounded-lg shadow-md overflow-hidden">
              <div className={`px-6 py-4 ${getStatusConfig().color} text-white`}>
                      ...
                      {sessions.filter(s => 
                        ...
                        <tr key={session.id} className="hover:bg-gray-50">
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-sm">
                            <span className={`px-3 py-1 rounded-full font-semibold ${getStatusConfig().color} text-white`}>
